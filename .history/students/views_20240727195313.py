...
def attendance_data_students(request):
    ...
    formatted_dates = [date.strftime('%Y-%m-%d') for date in [dates['date'] for dates in attendance_date ]]
    for datevar in attendance_date:
        students=Attendance.objects.filter(date=datevar['datetime__date'])
