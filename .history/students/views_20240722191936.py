...
def student_list(request):
    ...
    student_presenty={s.id : check_presenty(s.id) for s in students} 
    today=timezone.now().date()
    conetent={'students': students,"student_presenty":student_presenty ,"today":today}
    return render(request, 'students/home.html', conetent)
def check_presenty(id):
    today=timezone.now().date() 
    student=Student.objects.get(id=id)
    attendance=Attendance.objects.get(student= id ,date=today)
