...
def home(request):
    ...
    if isinstance(user, CustomUser):    
        ...
        user_type = user.user_type  eturn render(request,"students/home.html",content)
def attendance_data_students(request ,pk):
    ...
    # nested dictionary: {student_id: {date: status}}
    attendance_data = {
        student.id: {
...
def student_list(request,pk):  
    ...
    print(user_type)
    for student in students:
        attendance_exists = Attendance.objects.filter(Q(student=student) & Q(date=today) 
                                                & Q(student__classgroup=classgroup.id)).exists()
        student_attendance.append((student, attendance_exists)) 
    content={'student_attendance': student_attendance,"today":today ,"classgroup":classgroup} 
    return render(request, 'students/attendance.html', content) 
