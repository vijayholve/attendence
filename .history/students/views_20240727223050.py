...
from .task import send_mail_to_all_task ,send_mail_task ,send_mail_task_teacher send_mail_to_all_task_students
...
def attendance_data_students(request):
    ...
    start_date = end_date - timedelta(days=6)  # Adjust the range as needed
    ...
    attendance_data = {
        student.id: {
            date: Attendance.objects.filter(student=student, date=date).first()
...
def send_mails_to_students(request): 
    subject=request.POST.get('subject')  
    mail_text=request.POST.get('content')  
    if request.method == 'POST': 
