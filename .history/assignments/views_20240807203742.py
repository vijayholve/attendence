...
def all_classes(request):
    ...
    class1=Classes.objects.get(name="BA")
    years=ClassGroup.objects.filter(name=class1)
    year=request.GET.get('year')
    if class_id:=request.GET.get('class') and year:
        selected_class=Classes.objects.get(id=clid)
        classgroup=ClassGroup.objects.get(name=selected_class,year=year)
