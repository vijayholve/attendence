...
fromss_id and year_id:
        selected_class = get_object_or_404(Classes, id=class_id)
        selected_year = get_object_or_404(ClassGroup, id=year_id)
        # Assuming you have a view named 'home-assignments' that expects class and year IDs.
        return redirect('home-assignments', class_id=selected_class.id, year_id=selected_year.id)
    
    content = {"classes": classes, "years": years}
    return render(request,"assignements/assignemts_classes_list.html", content) .task import send_mail_to_all_students_for_assignemt 
import os 


def all_classes(request):
    classes = Classes.objects.all()
    years = ClassGroup.objects.all()
