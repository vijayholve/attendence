...
from django.db.models import Q  
from students.models import CustomUser ,Teacher,Student,Subject,admin 
...
def customize_login_page(request):
    username=request.POST.get('username')
    password=request.user = Users.objects.create_user(username=username, password=password)
    ...
    if user_type == 1:
        ...
        if request.method == "POST":
            ...
            if student_form.is_valid():
                return _extracted_from_user_panel_12(student_form, customeuser) 
            else:
                return redirect('register')
    elif user_type == '2':
        teacher_form = TeacherForm()
